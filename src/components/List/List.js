...
class List extends React.Component {
  ...
  render() {
    return (
      <WrapperPage>
        <PageTitle>starwars - heroes</PageTitle>
        {this.state.error ? (<Statment>Sorry, content is available, try again later . . .</Statment>) : (this.state.isLoading && <Statment>Wait,  I'm loading StarWars heroes for you . . . </Statment>)}
        {this.state.data.map(({ name, birth_year, gender, hair_color, eye_color, height, mass }) =>
          <Item
            key={name}
            name={name}
            birth={birth_year}
